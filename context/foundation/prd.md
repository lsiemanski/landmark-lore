...
- No importing from cloud photo services (Google Photos, iCloud, Dropbox) in v1 — device upload only. Rationale: cloud-source integrations require per-provider authorization flows that significantly widen scope.
...
1. **GDPR and image-rights compliance** — Server-side photo storage requires a data retention policy, a right-to-deletion flow, and a privacy notice before public launch. Owner: user. Block: yes for public launch (not for private testing).
   > Resolution (2026-05-31): Private testing with the owner's own photos is exempt under GDPR Recital 18 (personal/household use) — no compliance work needed before that phase. Before inviting any other user, the account deletion flow (Supabase Storage → metadata rows → auth user cascade) must exist as a technical prerequisite regardless of legal readiness. Before public launch, required artifacts are: Privacy Policy (GDPR Art. 13/14), Terms of Service (image rights + acceptable use), Supabase DPA (signed in their dashboard), AI provider data processing terms confirmed, and a data export endpoint (GDPR Art. 20). Cookie consent banner is only needed if non-essential cookies are added.
2. **Place/time tagging strategy for FR-008** — Should place (city/country) and time (month + year) come primarily from photo metadata with AI-inferred fallback for photos without metadata? Or should AI inference be the primary path? Owner: user. Block: no (affects FR-008/FR-009 which are nice-to-have).
3. **Follow-up question context (FR-005)** — Does the app retain the identification context between follow-up questions in a session (stateful), or does each question start fresh (stateless)? Stateful produces better answers but requires conversation memory. Owner: user. Block: no for v1 if stateless is acceptable.
